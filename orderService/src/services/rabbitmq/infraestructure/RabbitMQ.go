...
func (r *RabbitMQ) Close() {
	// Aquí se debe cerrar correctamente la conexión
	err := r.conn.Close()
	if err != nil {
		panic("Error closing RabbitMQ connection: " + err.Error())
	}
}
...
func (r *RabbitMQ) Publish(queue string, msg []byte) error {
	return r.ch.Publish(
		...
		false, // Immediate
		amqp.Publishing{
			ContentType: "application/json", // Cambiar el tipo de contenido a JSON
			Body:        msg,                // Enviar el mensaje JSON
		},
	)
}
